...
- **Prefer Spotify Links**: Background-resolves Spotify URIs for Last.fm tracks with staggered 300ms timing to avoid rate limits; clicking resolved tracks navigates to Spotify instead of Last.fm
...
- **Playlist artwork from RootlistAPI (Library)**: Supplemented `LibraryAPI.getContents()` (which returns empty `images` arrays) with a one-time `RootlistAPI.getContents({ flatten: true })` call to fetch playlist cover images, so playlists display their actual artwork collages instead of grey initial fallbacks
...
- [ ] Verify "Prefer Spotify Links" setting resolves Last.fm tracks to Spotify URIs in background
- [ ] Verify clicking resolved Last.fm tracks navigates to Spotify instead of Last.fm page
